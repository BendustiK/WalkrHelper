...
func MakeRequest() {
	...
	for _, requestBody := range requestMapping {
		// 获取传说列表
...
func _requestEpicList(requestBody RequestJson) (*http.Response, error) {
	...
	if req, err := http.NewRequest("GET", fmt.Sprintf("https://universe.walkrgame.com/api/v1/epics?%v", v.Encode()), nil); err == nil {
...
func _requestFleetList(requestBody RequestJson) (*http.Response, error) {
	...
	v.Add("name", "")
	v.Add("offset", "0")
	if req, err := http.NewRequest("GET", fmt.Sprintf("https://universe.walkrgame.com/api/v1/fleets?%v", v.Encode()), nil); err == nil {
		req.Header.Set("Cookie", requestBody.Cookie)
		if requestBody.IfNoneMatch != "" {
			req.Header.Add("If-None-Match", requestBody.IfNoneMatch)
		}
...
func _applyInvitedFleet(requestBody RequestJson, fleetId int) bool {
	...
	if req, err := http.NewRequest("POST", fmt.Sprintf("https://universe.walkrgame.com/api/v1/fleets/%v/apply", fleetId), bytes.NewBuffer([]byte(b))); err == nil {
		req.Header.Set("Cookie", requestBody.Cookie)
		if requestBody.IfNoneMatch != "" {
			req.Header.Add("If-None-Match", requestBody.IfNoneMatch)
		}
...
func _leaveComment(requestBody RequestJson, fleetId int, comment string) bool {
	...
	if req, err := http.NewRequest("POST", fmt.Sprintf("https://universe.walkrgame.com/api/v1/fleets/%v/comment", fleetId), bytes.NewBuffer([]byte(b))); err == nil {
		req.Header.Set("Cookie", requestBody.Cookie)
		if requestBody.IfNoneMatch != "" {
			req.Header.Add("If-None-Match", requestBody.IfNoneMatch)
		}
...
func _leaveFleet(requestBody RequestJson, fleetId int) bool {
	...
	if req, err := http.NewRequest("POST", fmt.Sprintf("https://universe.walkrgame.com/api/v1/fleets/%v/leave", fleetId), bytes.NewBuffer([]byte(b))); err == nil {
		req.Header.Set("Cookie", requestBody.Cookie)
		if requestBody.IfNoneMatch != "" {
			req.Header.Add("If-None-Match", requestBody.IfNoneMatch)
		}
...
func _getInvitationFleetId(resp *http.Response) int {
	...
	// 	if fleet.IsInvited == true {
	// 		fleetId = fleet.Id
	// 		log.Notice("舰队[%v:%v] by (%v): 正在邀请", fleet.Name, fleetId, fleet.Captain.Name)
	// 		return fleetId
	// 	}
	// }
